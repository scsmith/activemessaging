    ...
    class <<self
      ...
      def start
        ...
        begin
          ...
          ActiveMessaging.logger.error "Couldn't clean up correctly"
          ActiveMessaging.logger.error @@connection_threads.map { |name, thread| thread[:message].inspect }.join(' - ')
        ensure
      ...
      def load_connection_configuration(label='default')
        @broker_yml = YAML::load(ERB.new(IO.read(File.join(Rails.root, 'config', 'broker.yml'))).result) if @broker_yml.nil?
        if label == 'default'
